...
- MANDATORY occupancy gate after generating ANY deck: `npx tsx scripts/measure-occupancy.mts <rendered.html>` flags slides that leave a large empty band (the underfill tell). FIX every `UNDERFILL` slide (re-template/re-author, never just stretch thin content) and re-measure until all pass. Prose guidance alone does not hold — a deck is not done with any UNDERFILL slide. The worst offender is `flow-fill`+`flow-between` on a thin number/title/one-line card.
